    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                <div class="p-8">
                    <form method="POST" action="{{ route('variantes.update', $variante) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="notas" id="notas" rows="3"
                                ...
                                class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent transition">{{ old('notas', $variante->notas) }}</textarea>
                            @error('notas')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-6 p-4 bg-purple-50 rounded-xl border border-purple-200">
                            <h3 class="text-sm font-bold text-purple-900 mb-3">📊 Estadísticas Actuales</h3>
                            <div class="grid grid-cols-3 gap-4">
                                <div>
                                    <p class="text-xs text-purple-600 font-medium">Productos</p>
                                    <p class="text-2xl font-bold text-purple-900">{{ $variante->productos->count() }}</p>
                                </div>
                                <div>
                                    <p class="text-xs text-purple-600 font-medium">Ventas Totales</p>
                                    <p class="text-2xl font-bold text-purple-900">{{ $variante->ventas_totales }}</p>
                                </div>
                                <div>
                                    <p class="text-xs text-purple-600 font-medium">Stock Total</p>
                                    <p class="text-2xl font-bold text-purple-900">{{ $variante->stock_total }}</p>
                                </div>
                            </div>
                        </div>
                                        ...
                                        <div class="ml-3 flex-1">
                                            ...
                                            @if ($producto->ventas_totales > 0)
                                                <span class="ml-2 text-xs text-green-600 font-semibold">
                                                    {{ $producto->ventas_totales }} ventas
                                                </span>
                                            @endif
                                        </div>
                                    </label>
